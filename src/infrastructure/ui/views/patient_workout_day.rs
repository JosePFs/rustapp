...
pub fn PatientWorkoutDay(patient_program_id: String, day_index: String) -> Element {
    ...
    let data = use_resource(move || {
        ...
        async move {
            ...
            let pp_opt = backend.get_patient_program_by_id(sess.access_token(), &pid).await?;
            let pp = match pp_opt {
                Some(p) if p.status == "active" => p,
                _ => return Err("No se encuentra la asignación activa".to_string()),
            };

            let program = match backend.get_program(sess.access_token(), &pp.program_id).await? {
                Some(p) => p,
                None => return Err("Programa no encontrado".to_string()),
            };

            let schedule: Vec<ProgramScheduleItem> =
                backend.list_program_schedule(sess.access_token(), &pp.program_id)
                    .await
                    .unwrap_or_default();
            let workouts: Vec<Workout> =
                backend.list_workouts_for_program(sess.access_token(), &pp.program_id)
                    .await
                    .unwrap_or_default();
            let sessions: Vec<WorkoutSession> =
                backend.list_workout_sessions(sess.access_token(), &pp.id)
                    .await
                    .unwrap_or_default();
            ...
            let exercises =
                backend.list_exercises_for_workout(sess.access_token(), &workout_id).await?;
