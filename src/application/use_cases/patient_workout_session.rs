...
impl<B: Backend> PatientWorkoutSessionUseCase<B> {
    ...
    pub async fn execute(&self, args: PatientWorkoutSessionArgs) -> Result<PatientWorkoutSession> {
        ...
        let patient_program = match patient_program {
            ...
            _ => {
                return Err(DomainError::Api(t!("error_no_active_assignment").to_string()));
            }
        };
        ...
        let program = match program_opt {
            Some(p) => p,
            None => {
                return Err(DomainError::Api(t!("error_program_not_found").to_string()))
            }
        };
        ...
        let (_, workout_id_opt, label) = day_schedule
            ...
            .ok_or(DomainError::Api(
                t!("error_day_not_found").to_string(),
            ))?;
        ...
        let session_for_day = sessions
            .into_iter()
            .find(|s| s.day_index == args.day_index);

        let exercises_fut = self
            .backend
            .list_exercises_for_workout(token, &workout_id);
