...
use domain::repositories::{
    PatientSessionWriteRepository, SpecialistCatalogReadRepository,
    SpecialistCatalogWriteRepository,
};
use domain::{
    entities::{
...
impl SpecialistCatalogReadRepository for SupabaseRestRepository {
    ...
    async fn list_session_exercise_feedback_for_program(
        ...
        let body = serde_json::json!({ "p_patient_program_id": patient_program_id });
        let body_bytes = body.to_string().into_bytes();
        let response = self
            .client
            .rest_request(Some(access_token), "POST", path, Some(&body_bytes))
    ...
    async fn get_workout_with_exercises(
        ...
        let body = serde_json::json!({ "p_workout_id": workout_id });
        let body_bytes = body.to_string().into_bytes();
        let response = self
            .client
            .rest_request(Some(access_token), "POST", path, Some(&body_bytes))
    ...
    async fn get_program_with_agenda(
        ...
        let body = serde_json::json!({ "p_program_id": program_id });
        let body_bytes = body.to_string().into_bytes();
        let response = self
            .client
            .rest_request(Some(access_token), "POST", path, Some(&body_bytes))
    ...
    async fn get_specialist_dashboard(
        ...
        let body = serde_json::json!({ "p_specialist_id": specialist_id });
        let body_bytes = body.to_string().into_bytes();
        let response = self
            .client
            .rest_request(Some(access_token), "POST", path, Some(&body_bytes))
...
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
impl SpecialistCatalogWriteRepository for SupabaseRestRepository {
    ...
    async fn upsert_session_exercise_feedback(
        ...
        let _ = self.client.rest_delete(Some(access_token), &path).await;
        self.client
...
impl PatientSessionWriteRepository for SupabaseRestRepository {
    async fn get_or_create_session(
        ...
        SpecialistCatalogWriteRepository::get_or_create_session(
            self,
            access_token,
            patient_program_id,
            day_index,
    ...
    async fn uncomplete_session(&self, access_token: &str, session_id: &str) -> Result<()> {
        SpecialistCatalogWriteRepository::uncomplete_session(self, access_token, session_id).await
    }
